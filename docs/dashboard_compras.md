...
[Alfa Gestión SQL Server]
        ↓
[DashboardCompras.exe — ASP.NET Core / Kestrel]
        ↓
...
| Script | Descripción |
|---|---|
| `scripts\publicar_release.bat` | Publica la app en `publish\DashboardComprasLAN` |
...
2. Distribuir `publish\DashboardComprasInstaller\Output\DashboardComprasSetup_X.X.X.exe`
...
2. Copiar `publish\DashboardComprasLAN` al servidor
