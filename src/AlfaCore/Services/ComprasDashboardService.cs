...
public sealed partial class ComprasDashboardService(IConfiguration configuration, ILogger<ComprasDashboardService> logger, ISessionService sessionService) : IComprasDashboardService
{
    ...
    private readonly AlfaCore.Configuration.DatosSqlOptions _sqlOptions =
        configuration.GetSection(AlfaCore.Configuration.DatosSqlOptions.SectionName).Get<AlfaCore.Configuration.DatosSqlOptions>() ?? new();
